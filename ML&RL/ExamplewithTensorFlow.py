#This is a sample code making use of Tensorflow (Keras) to create a model.
...
class DQN:
    ...
    def build_model(self):
        ...
        model.add(Dense(128, input_dim=self.nS, activation='relu'))
    ...
    def replay(self):
        ...
        outstanding = np.where(minibatch[:, 4] == False)
        y = np.copy(minibatch[:, 2])

        # If minibatch contains any non-terminal states, use separate update rule for those states
        if len(outstanding[0]) > 0:
            predict_sprime = self.model.predict(np.vstack(minibatch[:, 3]))
            predict_sprime_target = self.target_model.predict(np.vstack(minibatch[:, 3]))
            
            # DQN update rule
            y[outstanding] += np.multiply(self.gamma, predict_sprime_target[outstanding, np.argmax(predict_sprime[outstanding, :][0], axis=1)][0])
